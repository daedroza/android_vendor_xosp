...
function xospapps_essentials() {
    ...
    function xospapps_essentials_dwnlderror {
        echo -e "Couldn't download ${APP_NAME}, please check your connection.";
        exit 0;
    }
    ...
    then
        ...
        echo -e "============";
            ...
            if [ -f ${APP_DIR}/${APP_NAME}.apk ]; then
                if [[ $(md5sum ${APP_DIR}/${APP_NAME}.apk | cut -f 1 -d " " | tr '[:lower:]' '[:upper:]') == $(curl ${XOSP_SITE}/${APP_DIR}/${APP_NAME}.apk.md5 | cut -f 1 -d "%") ]]; then
                       echo -e "${APP_NAME} is upto date";
                fi
                else {
                       echo -e "File doesn't exist. Downloading";
                       wget -t 2 ${XOSP_SITE}/${APP_DIR}/${APP_NAME}.apk -O ${APP_DIR}/${APP_NAME}.apk &>/dev/null || echo "Error downloading ${APP_NAME}";
                }
            fi
        done
        ...
        if [ ! -f ${APP_DIR}.zip ]; then
            wget -t 2 ${XOSP_SITE}/${APP_DIR}.zip -O ${APP_DIR}.zip &>/dev/null || xospapps_essentials_dwnlderror;
            unzip -q ${APP_DIR}.zip -d ${APP_DIR};
            echo -e "${APP_DIR} downloading complete.";
        fi
        if [[ $(md5sum ${APP_DIR}.zip | cut -f 1 -d " " | tr '[:lower:]' '[:upper:]') != $(curl ${XOSP_SITE}/${APP_DIR}.zip.md5 | cut -f 1 -d "%") ]]; then
            echo -e "${APP_DIR} downloading complete.";
        else {
            wget -t 2 ${XOSP_SITE}/${APP_DIR}.zip -O ${APP_DIR}.zip &>/dev/null || echo "Error downloading ${APP_DIR}";
            unzip -q ${APP_DIR}.zip -d ${APP_DIR};
            echo -e "${APP_DIR} downloading complete.";
        }
        fi
